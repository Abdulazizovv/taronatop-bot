...
@dp.message_handler(Command("top"), IsGroup(), state=None)
async def handle_group_trending(message: types.Message, state: FSMContext):
    """Handle /top command in groups to show trending music."""
    ...
    try:
        ...
        logging.info(f"User {message.from_user.id} requested trending music in group {message.chat.id}")
