...
# Primary method - simple and reliable
from bot.utils.instagram_simple import (
    download_instagram_media_simple as download_instagram_media,
    convert_instagram_video_to_audio_simple as convert_instagram_video_to_audio,
    get_instagram_media_info_simple as get_instagram_media_info,
    find_music_name
)
# Secondary fallback method
from bot.utils.instagram_fallback import (
    download_instagram_media as download_instagram_media_fallback,
    convert_instagram_video_to_audio as convert_instagram_video_to_audio_fallback,
)
...
async def handle_instagram_link(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        # Try primary method first (simple yt-dlp)
        ...
        if not result:
            await search_msg.edit_text("⏳ Fallback usuli ishlatilmoqda...")
            result = await download_instagram_media_fallback(search_query)
...
async def download_instagram_media_music(call: types.CallbackQuery, callback_data: dict):
    ...
    try:
        ...
        # Try fallback method if primary fails
        if not result:
            result = await convert_instagram_video_to_audio_fallback(media_info["video_url"])
